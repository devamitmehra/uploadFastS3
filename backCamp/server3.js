...
app.post('/generate-presigned-url', async (req, res) => {
  ...
  try {
    // Generate the presigned URL using AWS SDK v3
...
app.post('/generate-presigned-urls', async (req, res) => {
  const files = req.body.files; // Array of files from the client
  const presignedUrls = [];
  ...
  //try {
    for (const file of files) {
      const params = {
        Bucket: process.env.S3_BUCKET,
        Key: `uploads/${file.fileName}`, // Prepend folder to file name
        ContentType: file.fileType, // Set file type from the client
        ACL: 'public-read',
      };
